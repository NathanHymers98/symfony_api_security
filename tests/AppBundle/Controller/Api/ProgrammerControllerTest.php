...
EOF;
    ...
    public function testRequiresAuthentication() // Tests to see if when a user tries to do something they are not oauthorized to do, they get an error that tells them that they are not authenticated to do that action
    {
        $response = $this->client->post('/api/programmers', [ // Making a new request to /pai/programmers and am sending it a valid JSON body data
            'body' => '[]'
            // do not send auth!
        ]);
        $this->assertEquals(401, $response->getStatusCode()); // Assert that the response status code should be 401, meaning unauthorized
    }
